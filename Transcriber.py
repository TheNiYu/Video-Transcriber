...
def select_files():
    ...
    file_paths = filedialog.askopenfilenames(
        title="Seleziona i file da trascrivere",
        filetypes=[
            ("Video/Audio", "*.mp4 *.mkv *.mp3 *.wav"),
            ("Tutti i file", "*.*")
        ]
    )
...
def transcribe_video(file_path: str, model):
    ...
    if not video.exists():
        print(f"❌ File non trovato: {video}")
        return

    output_path = video.with_suffix(".txt")

    print(f"\n🎬 Trascrivo: {video.name}\n")

    segments, info = model.transcribe(
        str(video),
        beam_size=BEAM_SIZE,
        vad_filter=True,
        chunk_length=10  # 🔥 CRUCIALE
    )
    ...
    print(f"📝 Righe scritte: {lines_written}")
...
if __name__ == "__main__":
    print("📂 Seleziona i file da trascrivere...")
    ...
    print(f"\n📂 Selezionati {len(files)} file")

    print("\n🚀 Carico modello Whisper UNA VOLTA SOLA...")
    model = WhisperModel(
        MODEL_SIZE,
        device=DEVICE,
